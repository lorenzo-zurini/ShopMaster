...
void EMAGMdiChild::on_AuthRequestComplete(QNetworkReply * AuthReply)
{
    ...
    else
    {
        ...
        QSqlQuery DBQuery;
        DBQuery.exec("CREATE TABLE IF NOT EXISTS ORDERS "
                     "("
        ...
        for (int i = 0; i < OrdersArray.size(); i++)
        {
            DBQuery.exec("INSERT OR REPLACE INTO ORDERS "
                         ...
                         "VALUES "
                         "("
                        + QString::number(OrdersArray.at(i).toObject()["id"].toInt()) + ", "
